...
export default function Page() {
  ...
  async function send(text) {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: "assistant", text: "⚠️ Hata oluştu: " + err.message }]);
    }
    setLoading(false);
  }
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: #07090e; min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 20px; }
        .app { width: 100%; max-width: 780px; background: #0a0c10; border: 1px solid #1e2433; border-radius: 16px; overflow: hidden; display: flex; flex-direction: column; height: 92vh; max-height: 820px; box-shadow: 0 24px 80px rgba(0,0,0,0.6); font-family: 'DM Sans', sans-serif; color: #e2e8f0; }
